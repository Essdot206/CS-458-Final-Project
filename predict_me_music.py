...
def find_top_Musics(df, n=10):
    top_Musics = df.sort_values(by='predicted_salesrank').head(n)
    return top_Musics[['title', 'predicted_salesrank']]
...
if __name__ == "__main__":
    Musics_df = load_and_prepare_data('/Users/spencer/Desktop/CS-458-Fall-2023/Amazon Final Project/json/amazon-meta.json')
    asin_to_title = pd.Series(Musics_df.title.values, index=Musics_df.ASIN).to_dict()
    Musics_df['similar_titles'] = Musics_df['similar'].apply(lambda x: replace_asin_with_title(x, asin_to_title))
    Musics_df = predict_sales_rank(Musics_df)
    top_Musics = find_top_Musics(Musics_df)

    output_dir = '/Users/spencer/Desktop/CS-458-Fall-2023/Amazon Final Project/Excel'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    print("Top Predicted Musics:")
    print(top_Musics)
    top_Musics.to_excel('/Users/spencer/Desktop/CS-458-Fall-2023/Amazon Final Project/Excel/predicted_Musics.xlsx', index=False)
