...
def find_top_Videos(df, n=10):
    top_Videos = df.sort_values(by='predicted_salesrank').head(n)
    return top_Videos[['title', 'predicted_salesrank']]
...
if __name__ == "__main__":
    Videos_df = load_and_prepare_data('/Users/spencer/Desktop/CS-458-Fall-2023/Amazon Final Project/json/amazon-meta.json')
    asin_to_title = pd.Series(Videos_df.title.values, index=Videos_df.ASIN).to_dict()
    Videos_df['similar_titles'] = Videos_df['similar'].apply(lambda x: replace_asin_with_title(x, asin_to_title))
    Videos_df = predict_sales_rank(Videos_df)
    top_Videos = find_top_Videos(Videos_df)

    output_dir = '/Users/spencer/Desktop/CS-458-Fall-2023/Amazon Final Project/Excel'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    print("Top Predicted Videos:")
    print(top_Videos)
    top_Videos.to_excel('/Users/spencer/Desktop/CS-458-Fall-2023/Amazon Final Project/Excel/predicted_Videos.xlsx', index=False)
